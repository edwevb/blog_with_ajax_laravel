@extends('layouts.admin_layouts', ['title' => $tag->name])
@push('pageStyles')
...
<div class="container-fluid">
	...
	<h1 class="h3 mb-2 text-gray-800">Tag: {{$tag->name}}</h1>
			...
			<div class="card">
				<div class="card-body">
					...
					<div class="mt-3 small">
						...
						<a href="javascript:void(0)" id="removePost" class="card-link text-danger" name="{{$post->id}}"><i class="fa fa-trash"></i></a>
					</div>
				</div>
			</div>
		</div>
		@empty
		<h5 class="m-4">No posts has been made for {{$tag->name}}'s Tag</h5>
		@endforelse
	</div>
</div>
			...
			<div class="modal-body">
				<form id="postForm">
					...
					<div class="form-group">
						<select class="form-control" id="posts" name="posts" autocomplete="off" placeholder="Select posts">
...
<script>
	...
	$('#addPosts form').on('submit', function(e){
		e.preventDefault();
		const url = "{{ url('/admin/tags/add-post/'.$tag->id) }}",
		data = {
	...
	$(document).on('click', '#removePost',  function() {
		...
		url = "{{ url('/admin/tags/remove-post/'.$tag->id) }}",
		data = {
			_token:_token,
